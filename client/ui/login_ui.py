...
class LoginUI(Frame):
    ...
    def _create_widgets(self):
        # Card Container
        card = Frame(self, bg=Colors.BG_SIDEBAR, padx=40, pady=40)
        ...
        Label(card, text="💬", font=("Segoe UI Emoji", 50), 
              bg=Colors.BG_SIDEBAR, fg=Colors.ACCENT).pack(pady=(0, 10))

        Label(card, text="Zalo Dark", font=UISettings.FONT_TITLE, 
              bg=Colors.BG_SIDEBAR, fg="white").pack(pady=(0, 5))

        # Inputs
        Label(card, text="Tên hiển thị", font=("Segoe UI", 10, "bold"), 
              bg=Colors.BG_SIDEBAR, fg=Colors.TEXT_PRIMARY, anchor="w").pack(fill="x")
        self.username_entry = Entry(card, font=UISettings.FONT_NORMAL, width=30, 
                                    bd=1, relief=SOLID, bg=Colors.BG_MAIN, 
